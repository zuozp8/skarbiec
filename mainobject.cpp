...
void MainObject::dostalWiadomosc()
{
	...
	else if (inputThread->status.MPI_TAG == TAG_POTWIERDZENIE_ODBIORU)
	{
		czas_odbierania[inputThread->status.MPI_SOURCE] = inputThread->bufor[0];
	}
	inputThread->start();
	przetworzKolejke();
}
